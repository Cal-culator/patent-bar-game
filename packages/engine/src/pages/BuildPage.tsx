...
export function BuildPage() {
  ...
  const buildContent = content.getBuildContent(slug);
  ...
  if (absorbStatus !== "completed" && buildContent.fragments.length > 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <Link
          href={`/zones/${slug}`}
  ...
  if (buildContent.fragments.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <Link
          href={`/zones/${slug}`}
  ...
  const handleBuildersComplete = () => {
    if (buildContent.tables.length > 0) {
      setSubPhase("tables");
    } else {
      handlePhaseComplete();
    }
    ...
    <div className="max-w-2xl mx-auto px-4 py-8" data-zone={slug}>
      ...
      <AnimatePresence mode="wait">
        {subPhase === "builders" && (
          ...
          >
            <RuleBuilder
              builders={buildContent.fragments}
