...
import { RuleBuilderData } from "@/types";
    ...
    <div className="max-w-xl mx-auto">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="bg-white rounded-2xl p-4 border-2 border-b-4 border-[var(--color-border)] border-b-[var(--color-border-strong)] mb-4 min-h-[80px]">
            ...
            <div className="flex flex-wrap gap-1.5">
              {placedFragments.length === 0 ? (
                ...
                placedFragments.map((frag, i) => {
                  let borderClass = "border-[var(--color-border)] border-b-[var(--color-border-strong)]";
                  let bgClass = "bg-[var(--color-surface)]";
                  if (checked) {
                    if (builder.correctOrder[i] === frag.originalIndex) {
                      borderClass = "border-[var(--color-correct)] border-b-[var(--color-correct-shadow)]";
                      bgClass = "bg-[var(--color-correct-bg)]";
                    } else {
                      borderClass = "border-[var(--color-incorrect)] border-b-[var(--color-incorrect-shadow)]";
                  ...
                  return (
                    <motion.button
                      ...
                      className={`px-2.5 py-1.5 rounded-lg text-sm font-semibold border-2 border-b-4 transition-colors text-[var(--color-text-primary)] ${borderClass} ${bgClass}`}
            ...
            <div className="flex flex-wrap gap-1.5">
              {availableFragments.map((frag) => (
                <motion.button
                  ...
                  disabled={checked}
                  className="px-2.5 py-1.5 rounded-lg text-sm font-semibold border-2 border-b-4 border-[var(--color-border)] border-b-[var(--color-border-strong)] hover:bg-[var(--color-surface)] transition-colors disabled:opacity-30 active:border-b-2 active:translate-y-[2px]"
              ...
              {availableFragments.length === 0 && !checked && (
                <span className="text-xs font-semibold text-[var(--color-text-muted)] italic">
          ...
          {!checked ? (
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-2.5 rounded-xl text-sm font-bold border-2 border-b-4 border-[var(--color-border)] border-b-[var(--color-border-strong)] hover:bg-[var(--color-surface)] transition-colors disabled:opacity-30 active:border-b-2 active:translate-y-[2px]"
              >
                Clear
              </button>
              <button
                onClick={handleCheck}
                disabled={!allPlaced}
                className={`flex-1 py-2.5 rounded-xl font-bold text-sm border-2 border-b-4 transition-colors disabled:opacity-40 uppercase tracking-wide active:border-b-2 active:translate-y-[2px] ${
                  allPlaced
            ...
            >
              <div className={`rounded-2xl p-4 text-sm mb-3 ${isCorrect ? "bg-[var(--color-correct-bg)]" : "bg-[var(--color-incorrect-bg)]"}`}>
